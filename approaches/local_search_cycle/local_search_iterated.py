...
class LocalSearchIterated(LocalSearch):
    def solve_ils1(self, method = 'steepest', num_moves = 12, max_time = 10):
        if self._first_solution is not None and self._second_solution is not None:
            return self._ils1_fn(method, num_moves, max_time)
        else:
            print(f'Ustaw wartości cykli.')

    def solve_ils2(self, method = 'steepest', num_nodes = 12, max_time = 10):
        if self._first_solution is not None and self._second_solution is not None:
            return self._ils2_fn(method, num_nodes, max_time)
        else:
            print(f'Ustaw wartości cykli.')

    def _ils1_fn(self, method, num_moves, max_time):
        cost = self.compute_total_cost()
        solutions_list = list()

        start = time.time()
        _, cost = self._steepest_solution(method, max_time = max_time - (time.time() - start), total_best_cost = cost)
    ...
    def _ils2_fn(self, method, num_nodes, max_time):
        cost = self.compute_total_cost()
        solutions_list = list()
        ...
        while time.time() - start < max_time:
            ...
            random_perturbation = random.sample(range(int(self.examples_num_first)), k = num_nodes)
